...
from babysteps.ci_adapter import GitHubCIAdapter
from babysteps.cli_helpers import find_git_root
from babysteps.command_runner import (
    CommandRunnerWithEnvLookUp,
    OSCommandRunner,
    OSEnvAdapter,
)
from babysteps.config_adapter import TOMLConfigAdapter
from babysteps.parsing import parse_numeric_input
from babysteps.prober import Prober, StaticDateProvider, local_system_date_provider
from babysteps.ratchet import (
    Ratchet,
...
app = typer.Typer(
    help="Baby Steps: Monotonic progress tracking and enforcement.",
    no_args_is_help=True,
)

DEFAULT_CONFIG_FILE = Path("babysteps.toml")
...
def add(
    ...
    """Adds a ratchet to your configuration (Auto-creates babysteps.toml if missing)."""
...
def ci(
    ...
    config: Path | None = typer.Option(
        None, "--config", "-c", help="Custom path to the babysteps configuration file."
    ),
):
    ...
    action_path = Path(".github/actions/babysteps/action.yml")
    if action_path.exists() and not overwrite:
        typer.secho(
            ...
            f"uv run babysteps ci --overwrite",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1)
